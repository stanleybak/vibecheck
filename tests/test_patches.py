...
from vibecheck.core.linmap import Conv2d
from vibecheck.core.patches import PatchAdjoint
...
def test_patch_refine_parity_conv_relu_conv():
    ...
    from vibecheck.core.patches import patch_refine
...
def test_lattice_flip_search_sign_net():
    ...
    from vibecheck.core.linmap import Dense, ScaleShift
    from vibecheck.frontend.spec import VNNSpec, Conjunct, Constraint
...
def test_point_taps_capture_sign_inputs():
    ...
    from vibecheck.core.linmap import ScaleShift, Dense
    n = 3
    sh = ScaleShift(None, np.full(n, -2.5, dtype=np.float32), n)
    agg = Dense(np.ones((1, n), dtype=np.float32), None)
...
def test_patch_refine_parity_through_pool_tree():
    ...
    import torch
    from vibecheck.core.graph import Net, Op, decompose_maxpool
    from vibecheck.core import backward
    from vibecheck.core.patches import patch_refine
