#!/bin/bash
# VNNCOMP prepare_instance.sh for vibecheck2 (vc2). UNTIMED, runs once before
...
stale() { pgrep -f 'vibecheck2\.verify' >/dev/null 2>&1; }
pkill -f 'vibecheck2\.verify' 2>/dev/null || true
w=0; while stale && [ $w -lt 20 ]; do pkill -9 -f 'vibecheck2\.verify' 2>/dev/null || true; sleep 1; w=$((w+1)); done
