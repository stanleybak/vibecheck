...
def stabilize_intermediates(net, W, lo1, hi1, inter, budget, device='cpu',
    ...
    for ps in range(passes):
        ...
        n_unst = sum(int(((inter[nm][0] < 0) & (inter[nm][1] > 0)).sum())
                     for nm in relu_edges)
        log(f'[vibecheck/stab] pass={ps + 1} unstable={n_unst}')
...
def input_split_bab(net, spec, W, bias, disj_idx, lo, hi, deadline,
    ...
    if roots is not None:
        ...
        log(f'[vibecheck/bab] multi-sub: {R_lo.shape[0]} roots over '
    ...
    msub_nocrown = (roots is not None
                    ...
                    ('', '0'))
    if msub_nocrown:
        log('[vibecheck/bab] multi-sub forward-only bound regime '
            '(mul+reciprocal family)')
    if z_rad_mode:
        log('[vibecheck/bab] dense zono projected over budget at B=16; '
    ...
    if full_refine and not forced_refine:
        ...
        try:
            ...
            log(f'[vibecheck/bab] refine probe: crown={c_lb:.3f} zono={z_lb:.3f} '
    ...
    while f_lo.shape[0] or (pend_lo is not None and pend_lo.shape[0]):
        ...
        if (rounds > 4 and batch < 65536 and f_lo.shape[0] > 4 * batch
            ...
            # (adaptive batch REGIME selection was tried here -- leaf-cost
            # 3e-5 / frontier>batch / drain-gated variants -- and every
            # rule helped one acasxu row while exploding another (4_7-p1
            # needs 65536 full-frontier pops: 215k boxes vs 2.6M timeout;
            # 3_3-p2 needs 4096 worst-first discipline: 13s vs 10M-box
            # explosion). The caller's two-stage escalation now carries
            # the regime instead: stage 1 = 4096 disciplined, stage 2 =
            # 65536 throughput. This trigger stays exactly as validated
            # by the 123/123 sweep.)
            batch *= 2
        _round_t0 = time.time()
        ...
        try:
            if full_refine:
                ...
                try:
                    ...
                    if not z_rad_mode:
                        z_rad_mode = True
                        log('[vibecheck/bab] dense zono OOM; switching to '
                ...
                if ib is None and zono_oom and (roots is None
                    ...
                    try:
                        ...
                        if os.environ.get('VC2_DEBUG_CLIP'):
                            log(f'[vibecheck/bab] round={rounds} chunked '
        ...
        # (an f64 "knife-edge rebound" of near-zero domains was tried
        # here -- window/cap/leaf-width gated variants all included.
        # MEASURED: it closed ZERO boxes on every family (the pinned
        # dual roots are relaxation-limited at -2e-3, and the deep
        # leaves close by splitting anyway) while its per-round f64
        # pass cost ~0.3s x 20+ firings and flipped the T=20
        # knife-edge nn4sys 0_500 row to unknown. Reverted; do not
        # re-add without a case it demonstrably closes.)
        ...
        if os.environ.get('VC2_DEBUG_CLIP') and rounds <= 60:
            ...
            log(f'[vibecheck/round] round={rounds} popped={blo.shape[0]} '
        ...
        if (os.environ.get('VC2_DEBUG_GAP') and brow is not None
            ...
            if bool(_om.any()):
                ...
                log(f'[vibecheck/gap] round={rounds} open={int(_om.sum())} '
        ...
        try:
            ...
            if open_mask.any() and alpha_iters > 0 and oi.numel():
                ...
                if os.environ.get('VC2_DEBUG_CLIP') and rounds % 16 == 1:
                    log(f'[vibecheck/alpha] round={rounds} open_pre={oi.numel()} '
        ...
        if open_mask.any():
            ...
            if os.environ.get('VC2_DEBUG_CLIP') and rounds % 16 == 1:
                ...
                w_post = ((torch.where(feas_any.unsqueeze(1), xh_u, ohi)
                          ...
                          .clamp(min=0).sum())
                log(f'[vibecheck/clip] round={rounds} open={olo.shape[0]} '
            ...
            if fdev.type == 'cuda':
                ...
                if fbytes > 0.10 * memory.free_bytes(dev):
                    ...
                    log(f'[vibecheck/bab] frontier -> host '
            ...
            if onnx_path is not None and (
                ...
                if os.environ.get('VC2_DEBUG_CLIP') and rounds <= 60:
                    log(f'[vibecheck/bab] round={rounds} in-bab attack '
        ...
        if rounds % 32 == 0:
            log(f'[vibecheck/bab] round={rounds} frontier={int(f_lo.shape[0])} '
...
def relu_split_bab(net, spec, W, bias, disj_idx, lo, hi, deadline,
    ...
    while heap:
        ...
        if os.environ.get('VC2_DEBUG_CLIP') and rounds <= 6:
            log(f'[vibecheck/rbab] round={rounds} start B={B} bs={bs} '
        ...
        try:
            ...
            if bound == 'zono':
                ...
                if _dbg:
                    ...
                    log(f'[vibecheck/rbab] round={rounds} zono {time.time()-_tz:.2f}s B={B}')
                    _tz = time.time()
                ...
                if _dbg:
                    torch.cuda.synchronize() if dev.type == 'cuda' else None
                    log(f'[vibecheck/rbab] round={rounds} scoring {time.time()-_tz:.2f}s '
            ...
            if batch <= 1:
                return 'timeout', {'frontier': len(heap),
                                   ...
                                   'reason': 'oom_floor'}
            batch = max(1, B // 2)
            log(f'[vibecheck/rbab] round={rounds} OOM at B={B}; batch -> {batch}')
            continue
        n_bounded += B
        ...
        if open_mask.any():
            ...
            if os.environ.get('VC2_DEBUG_CLIP') and rounds <= 8:
                ...
                log(f'[vibecheck/rbab] round={rounds} splits: '
                    + ' '.join(_sd))
        ...
        if rounds % 16 == 0 or (bound == 'zono' and rounds % 4 == 0):
            ...
            log(f'[vibecheck/rbab] round={rounds} frontier={len(heap)} '
