...
def pgd(net, spec, lo=None, hi=None, restarts=64, iters=100, seed=0,
    ...
    for it in range(iters):
        ...
        if _check and (best_m < 0).any():
            ...
            deepen_iters += 1
            if float(best_m.detach().min()) < -1e-2 or deepen_iters > 25:
                break
    ...
    info = {'best_margin': float(best_m.detach().min()), 'iters': it + 1,
            'time': time.time() - t0,
            ...
            .astype(np.float64)}
    log(f'[vibecheck/pgd] best_margin={info["best_margin"]:+.3e} '
    ...
    if best_m[order[0]] > 0:
        ...
        if has_sign and float(span.max()) >= 2.0 \
            ...
            if fm < float(best_m.detach()[order[0]]) and fx is not None:
                best_m[order[0]] = fm
                best_x[order[0]] = fx
    if best_m[order[0]] <= 0:
        return best_x[order[0]].detach().cpu().numpy().astype(np.float64), info
    bm = float(best_m.detach()[order[0]])
    if 0.0 < bm < 1e-2:
        ...
        for bi in order[:polish_tries].tolist():
            if float(best_m.detach()[bi]) >= 1e-2:
                break
...
def _slp_polish(net, spec, x0, lo, hi, device, iters=40, time_budget=8.0,
    ...
    for _ in range(iters):
        ...
        if fn < fm:
            ...
            r *= 0.4
    log(f'[vibecheck/pgd] slp-polish: margin={fm:+.3e} t={time.time() - t0:.2f}s')
...
def _lattice_flip_search(net, spec, lo1, hi1, x0s, dev, margins,
    ...
    for x0 in x0s:
        ...
        if best_m <= 0:
            break
    log(f'[vibecheck/pgd] lattice-flip: best_margin={best_m:+.3e} '
    ...
    if (np.all(x_hi - x_lo >= 0)
        ...
        if not np.allclose(snapped, np.asarray(witness, dtype=np.float64)):
            ok_s, info_s = _validate_sat_witness(onnx_path, spec, snapped,
                                                 atol=1e-4, out_atol=0.0)
            if ok_s:
                log('[vibecheck] lattice-snapped candidate validated strictly')
                return ok_s, info_s
    if CE_TOL > 0:
        ...
        if ok_tol:
            ...
            log(f'[vibecheck] within-tolerance witness stashed (margin <= '
