...
def resolve_saturation(mode='auto', log=print):
    ...
    mode = str(mode).lower()
    if mode in ('on', 'true'):
        log('[vibecheck/surrogate] saturation: ON (forced)')
        return True
    if mode in ('off', 'false'):
        log('[vibecheck/surrogate] saturation: OFF (forced)')
        return False
    oracle = detect_quant_oracle()
    sat = (oracle == 'saturating')
    log(f'[vibecheck/surrogate] saturation: {"ON" if sat else "OFF"} '
...
def surrogate_attack(onnx_path, vnnlib_path, timeout, device='cpu',
    ...
    if device == 'cuda':
        ...
        log('[vibecheck/surrogate] GPU grad path unvalidated in this port; '
            'using CPU')
    dev = 'cpu' 
    if saturate:
        ...
        if not os.path.exists(fq_path):
            ...
            log(f'[vibecheck/surrogate] built fakequant surrogate -> {fq_path}')
        ...
        if not os.path.exists(surrogate_path):
            ...
            log(f'[vibecheck/surrogate] built float surrogate -> {surrogate_path}')
        model = convert(surrogate_path).eval().to(dev)
        eval_model = None
        if not os.path.exists(fq_path):
            build_fakequant_surrogate(onnx_path, fq_path)
        eval_model = convert(fq_path).eval().to(dev)
    log(f'[vibecheck/surrogate] loaded on {dev} in {time.time() - t0:.1f}s; '
    ...
    def ort_consider(pts, tag):
        ...
        m = margin_np(y)
        if m >= strict_buffer:
            log(f'[vibecheck/surrogate] CLEAR SAT at {tag} (ORT margin={m:.3e})')
            return feed
        return None
    ...
    log(f'[vibecheck/surrogate] no CE (t={time.time() - t0:.1f}s; '
...
def try_quant_surrogate(onnx_path, vnnlib_path, timeout, device='cpu',
    ...
    t0 = time.time()
    spec = parse_box_and_output(vnnlib_path)
    log(f'[vibecheck/surrogate] quantized net; spec parsed in '
        f'{time.time() - t0:.1f}s '
    ...
    ok, vinfo = _validate_witness_ort(onnx_path, wits, boxes, _violated,
                                      1e-4)
    if not ok:
        log(f'[vibecheck/surrogate] witness failed the unified ORT gate '
