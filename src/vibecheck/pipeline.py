"""vibecheck verification entry point.
...
def _peel_output_softmax(net, spec, log):
    ...
    log('[vibecheck] terminal softmax peeled (pure-difference spec rows: the '
...
def verify(onnx_path, vnnlib_path, timeout=60.0, device='cpu',
    ...
    try:
        ...
        log(f'[vibecheck] graph load failed ({type(e).__name__}: {str(e)[:80]}); '
        ...
        if isinstance(e, NotImplementedError):
            log(f'[vibecheck] net not supported: {e}; verdict unknown')
            return 'unknown', {
    ...
    try:
        ...
        try:
            ...
            pass
        log(f'[vibecheck] spec not supported: {e}; verdict unknown')
    ...
    try:
        ...
        log('[vibecheck] budget exhausted (OutOfTime); verdict unknown')
        ...
        # honest verdict
        log(f'[vibecheck] not implemented: {e}; verdict unknown')
        return 'unknown', {'reason': f'not_implemented: {e}',
                           'time': time.time() - t0}
        ...
        log(f'[vibecheck] cuda oom/accelerator failure: {str(e)[:80]}; '
            f'verdict unknown')
...
def _verify_groups(net, spec, groups, onnx_path, timeout, device,
    ...
    if len(groups) == 1:
        return _verify_one(net, spec, onnx_path, timeout, device,
                           alpha_iters, pgd_budget, log, t0)
    log(f'[vibecheck] {len(groups)} input-subbox groups (per-disjunct boxes)')
    if len(groups) > 16:
        ...
        if all(len(per_d.get(d, ())) == 1
            ...
            log(f'[vibecheck] multi-sub bab: {verdict} '
            ...
            if binfo.get('tol_witness') is not None:
                log('[vibecheck] verdict unknown BUT a within-tolerance CE was '
    ...
    if len(groups) > 16:
        ...
        log(f'[vibecheck] {len(groups)} groups open after batched screening')
...
def _verify_one(net, spec, onnx_path, timeout, device, alpha_iters,
    ...
    if pgd_budget > 0:
        ...
        for _ in range(3):                        # 100 -> 250 -> 625
            ...
            if w is not None:
                ...
                log('[vibecheck] pgd candidate rejected by ORT chokepoint; '
                    'continuing')
        ...
        if relu_only and 0.0 < ainfo['best_margin'] < 1e-2 \
            ...
            if w is not None:
                ...
                if vinfo.get('within_tol_witness') is not None:
                    tol_w = vinfo['within_tol_witness']
                log('[vibecheck] f64 fine-descent candidate rejected by ORT '
    ...
    def _phase(tag, lbv):
        ...
        verdict, open_d = _verdict_from_lbs(lbv + b, di,
                                            len(spec.disjuncts))
        log(f'[vibecheck] {tag}: worst={float((lbv + b).min()):.4f} '
    ...
    def _try_milp(open_d, deadline):
        ...
        try:
            ...
            log(f'[vibecheck/milp] skipped: {e}')
            return 'open', open_d
        if cand is not None and onnx_path is not None:
            ...
            if vinfo.get('within_tol_witness') is not None:
                tol_w = vinfo['within_tol_witness']
            log('[vibecheck/milp] incumbent rejected by ORT chokepoint')
        rem = [d for d in open_d if int(d) not in mrefuted]
        log(f'[vibecheck] milp: {len(mrefuted)} disjuncts refuted, {len(rem)} open')
    ...
    if verdict != 'unsat' and open_d and wide_route:
        ...
        for ai, frac, sbatch in stages:
            ...
            log(f'[vibecheck] input_split_bab (wide slice, alpha={ai}): '
                f'{verdict} '
                f'{ {k: v for k, v in binfo.items() if k != "witness"} }')
            if verdict == 'sat':
                return 'sat', {'witness': binfo['witness'],
    ...
    if verdict != 'unsat' and budget.remaining() > 15 \
        ...
        try:
            ...
            if lb_f is None:
                log('[vibecheck] fzono-alpha skipped (oom)')
            else:
            ...
            log(f'[vibecheck] fzono-alpha skipped ({e})')
    if verdict != 'unsat' and len(open_d) == 1 and n_nonlin <= 80000:
        ...
        try:
            ...
            log(f'[vibecheck] zono-lift skipped ({e})')
    ...
    if verdict != 'unsat' and budget.remaining() > 5:
        ...
        log(f'[vibecheck] dual-lp: {len(refuted)} disjuncts refuted, '
            f'{len(open_d)} open')
        if not open_d:
            return 'unsat', {'time': time.time() - t0}
    if verdict != 'unsat' and open_d and milp_eligible \
    ...
    if verdict != 'unsat' and budget.remaining() > 3:
        ...
        if ckpt:
            ...
            log(f'[vibecheck] BaB checkpoint saved to {ckpt}')
        verdict, binfo = bab(
            net, spec, W, b, di, lo[0], hi[0],
            deadline=t0 + timeout - 2.0, device=device,
            onnx_path=onnx_path, log=log, **kw)
        log(f'[vibecheck] {bab.__name__}: {verdict} '
            f'{ {k: v for k, v in binfo.items() if k != "witness"} }')
        if verdict == 'sat':
            return 'sat', {'witness': binfo['witness'],
                           'time': time.time() - t0}
    ...
    if (verdict != 'unsat' and pgd_budget > 0 and onnx_path is not None
        ...
        while budget.remaining() > 4:
            ...
            if w is not None:
                ...
                if vinfo.get('within_tol_witness') is not None:
                    tol_w = vinfo['within_tol_witness']
                log('[vibecheck/sign] candidate rejected by ORT chokepoint; '
                    'next frac pass')
            p_i += 1
        log(f'[vibecheck/sign] endgame exhausted after {p_i} frac passes')
    ...
    if verdict != 'unsat' and tol_w is not None:
        ...
        log('[vibecheck] verdict unknown BUT a within-tolerance CE was found '
...
def main(argv=None):
    ...
    if argv and (argv[0] in ('--name', '--version', '--examples-dir',
                             'supports', 'verify')
...
def _quiet_third_party_warnings():
    """Silence the benign, non-actionable third-party noise a verification
    run floods stderr with (v1 port): the numpy 'not writable' UserWarning
    (LinMap wraps ONNX weight buffers without copying) and onnxruntime's
    GPU device-discovery warnings. The latter are emitted from ORT's C++
    layer at its FIRST import (the Python logger-severity API does not gate
    them), so that first import happens here with fd 2 redirected to
    /dev/null; every later `import onnxruntime` reuses the cached module.

    Called lazily from `_run_flat` (NOT from `import vibecheck`), so a bare
    import — or `vibecheck --version` — stays fast and neither loads
    onnxruntime nor mutates global warning filters. Runs once; opt out with
    VIBECHECK_SHOW_WARNINGS=1. vibecheck's own progress is unaffected."""
    global _THIRD_PARTY_WARNINGS_QUIETED
    if _THIRD_PARTY_WARNINGS_QUIETED \
            or os.environ.get('VIBECHECK_SHOW_WARNINGS'):
        return
    _THIRD_PARTY_WARNINGS_QUIETED = True
    import warnings
    warnings.filterwarnings('ignore', message='.*not writable.*')
    # fd-level stderr redirect around the onnxruntime import only; any fd
    # failure (stderr closed / fd exhaustion) falls back to a plain import,
    # never leaking a descriptor and never leaving stderr redirected.
    _fd2 = _devnull = None
    try:
        _fd2 = os.dup(2)
...
def _make_flat_parser():
    """The flat (legacy) CLI parser; shared by `_legacy_main` and the
    programmatic `api.verify` so both drive the identical pipeline."""
    import argparse
    p = argparse.ArgumentParser(prog='vibecheck')
...
def _legacy_main(argv=None):
    ...
    _verdict, _details, code = _run_flat(a)
    return code


def _run_flat(a):
    """Run one verification from a parsed flat-CLI namespace: the single
    pipeline behind `_legacy_main` and `api.verify`. Handles network-pair
    merging, results-file pre-seed/write (the verdict authority), and the
    OOM->unknown / crash->error dispositions. Returns
    (verdict, details, exit_code)."""
    _quiet_third_party_warnings()
    if a.device is None:
        a.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        print(f'[vibecheck] device auto-selected: {a.device}')
    ...
    try:
        ...
        if a.results_file:
            with open(a.results_file, 'w') as f:
                f.write('unknown\n')
        print(f'[vibecheck] cuda oom (top-level): verdict unknown ({str(e)[:80]})')
        return 'unknown', {'time': time.time() - t0,
                           'error': f'cuda-oom: {str(e)[:300]}'}, 1
        ...
        if 'out of memory' in str(e).lower():
            if a.results_file:
                with open(a.results_file, 'w') as f:
                    f.write('unknown\n')
            print('[vibecheck] oom-message runtime error (top-level): unknown')
            return 'unknown', {'time': time.time() - t0,
                               'error': f'oom: {str(e)[:300]}'}, 1
        traceback.print_exc()
        if a.results_file:
            with open(a.results_file, 'w') as f:
                f.write(f'error\n{type(e).__name__}: {str(e)[:300]}\n')
        return 'error', {'time': time.time() - t0,
                         'error': f'{type(e).__name__}: {str(e)[:300]}'}, 2
    if a.results_file:
        ce = None
