...
cli_standard (main 35f6931), same surface:
    ...
    vibecheck2 --name                      # tool identifier
    vibecheck2 --version                   # tool version
    vibecheck2 verify <query.vnnlib> --network NAME=model.onnx [--timeout N]
                      [--serialise-assignments DIR]
    vibecheck2 supports <capability>       # e.g. --onnx-operators
...
def run_supports(argv):
    """`vibecheck2 supports <capability>...` -- print supported identifiers,
...
def run_verify(argv):
    """`vibecheck2 verify <query> [--network NAME=PATH]... [--timeout N]` --
    verify through the legacy pipeline with strict standard stdout."""
    parser = argparse.ArgumentParser(
        prog='vibecheck2 verify',
    ...
    if results_file is None:
        fd, tmp_rf = tempfile.mkstemp(prefix='vibecheck2_results_',
                                      suffix='.txt')
        os.close(fd)
        results_file = tmp_rf
